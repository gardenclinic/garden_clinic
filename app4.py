...
# ─────────────────────────────────────────────
    ...
    with col1:
        st.markdown(card("Gross Revenue", f"${gross_income:,.2f}", "green"), unsafe_allow_html=True)
    with col2:
        st.markdown(card("Total Outflows", f"${total_outflows:,.2f}", "red"), unsafe_allow_html=True)
    with col3:
        st.markdown(card("Net Profit", f"${net_profit:,.2f}", "dark"), unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
    ...
    with s2:
        ...
        with col3:
            emp_salary = st.number_input("Monthly salary ($)", min_value=0.0, step=100.0)
    ...
    with s3:
        ...
        with col3:
            s_price = st.number_input("Price ($)", min_value=0.0, step=10.0)
