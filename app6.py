...
# Dynamic left bottom footer injection
...
if selected == "📈 Dashboard":
    ...
    recent_logs = fetch_all("SELECT timestamp as Timestamp, user as [User Agent], action as [Action Executed] FROM system_logs ORDER BY id DESC LIMIT 15")
    if recent_logs:
        st.dataframe(pd.DataFrame([dict(l) for l in recent_logs]), use_container_width=True, hide_index=True)
    else:
        st.info("No operational logs recorded yet.")
        ...
        if not docs_db or (not services_db and not bundles_db):
            ...
            if st.button("Save & Print Receipt", use_container_width=True):
                if target_p == "— select —":
                    ...
                    log_activity(f"Checked out patient '{target_p}' for item '{chosen_item_name}' (Paid: {final_due:,.0f} IQD).")
        ...
        if all_p:
            ...
            if st.button("Remove Patient", type="primary"):
                if del_target != "— select —":
                    ...
                    log_activity(f"Removed patient record completely: '{del_target}'")
        ...
        if st.button("Register Patient"):
            if p_name.strip():
                if execute_write("INSERT INTO patients (name, phone, date_of_birth, gender, notes, created_at) VALUES (?,?,?,?,?,?)", (p_name.strip(), p_phone.strip(), p_dob.strip(), p_gender, p_notes.strip(), today_str)):
                    log_activity(f"Registered new patient profile: '{p_name.strip()}'")
    ...
    with t5:
        ...
        if all_visits:
            ...
            if st.button("Delete Visit", type="primary"):
                ...
                log_activity(f"Deleted customer checkout ticket index ID: #{void_id}")
    ...
    with ta1:
        ...
        if not patients_db or not docs_db:
            ...
            if st.button("Book Appointment"):
                ...
                log_activity(f"Scheduled appointment for '{ap_patient}' with Doctor '{ap_doctor}' on {ap_date}.")
    ...
    with ta2:
        ...
        if all_appts:
            ...
            if st.button("Update Status"):
                ...
                log_activity(f"Updated appointment dynamic ID #{upd_id} to status: '{new_status}'")
        ...
        with st.form("expense_form"):
            ...
            if st.form_submit_button("Add Expense"):
                if e_desc and e_amt > 0:
                    ...
                    log_activity(f"Logged dynamic business expense: '{e_desc}' ({e_amt:,.0f} IQD)")
...
# ─────────────────────────────────────────────
    ...
    page_header("Settings", "Configure doctors, staff, services, bundles, and system access profiles.")
    
    tabs_list = ["Doctors", "Staff & Payroll", "Services", "Bundles"]
    if role == "Boss":
        ...
        if st.button("Add Doctor"):
            if d_name.strip():
                if execute_write("INSERT INTO doctors (name, specialty, comm_type, fixed_rate) VALUES (?,?,?,?)", (d_name.strip(), d_spec.strip(), comm_type, f_rate)):
                    log_activity(f"Added clinical doctor profile: '{d_name.strip()}'")
        ...
        if all_docs:
            ...
            if st.button("Remove Doctor", type="primary"):
                if del_doc != "— select —":
                    ...
                    log_activity(f"Removed clinic doctor reference: '{del_doc}'")
        ...
        if st.button("Add Staff Member"):
            if emp_name.strip() and emp_role.strip():
                if execute_write("INSERT INTO employees (name, role, salary) VALUES (?,?,?)", (emp_name.strip(), emp_role.strip(), emp_salary)):
                    log_activity(f"Added internal employee member: '{emp_name.strip()}'")
        ...
        if all_emp:
            ...
            if st.button("Remove Employee", type="primary"):
                if del_emp != "— select —":
                    ...
                    log_activity(f"Dropped employee profile from staff records: '{del_emp}'")
        ...
        if st.button("Add Service"):
            if s_name.strip():
                if execute_write("INSERT INTO services (name, category, price, active) VALUES (?,?,?,1)", (s_name.strip(), s_cat, s_price)):
                    log_activity(f"Created medical treatment service option: '{s_name.strip()}' at {s_price:,.0f} IQD")
        ...
        if all_svc:
            ...
            if st.button("Remove Service", type="primary"):
                if del_svc != "— select —":
                    ...
                    log_activity(f"Removed clinic procedure entry: '{del_svc}'")
        ...
        if st.button("Create Bundle"):
            if b_name.strip() and b_price > 0:
                if execute_write("INSERT INTO bundles (name, price, description) VALUES (?,?,?)", (b_name.strip(), b_price, b_desc.strip())):
                    log_activity(f"Configured multi-item catalog bundle package: '{b_name.strip()}'")
        ...
        if all_bundles:
            ...
            if st.button("Remove Bundle", type="primary"):
                if del_bnd != "— select —":
                    ...
                    log_activity(f"Removed treatment package catalog bundle: '{del_bnd}'")
                    st.success(f"Bundle '{del_bnd}' removed.")
                    st.rerun()
        else:
            st.info("No bundles created yet.")

    # ── ACC COUNTS & CREDENTIAL MODIFICATION (BOSS ONLY) ──
    if role == "Boss":
        with s_tabs[4]:
            section_label("System Registration Control Desk")
            users_list = fetch_all("SELECT id, username, role, plaintext_password FROM users ORDER BY role, username")
            
            # Displays the real-time card metric count
            st.metric("Total Authorized Accounts Registered", len(users_list))
            
            # Formatted list table displaying plain passwords
            raw_user_data = []
            for usr in users_list:
                raw_user_data.append({
                    "System Account ID": usr["id"],
                    "Username": usr["username"],
                    "Assigned Control Role": usr["role"],
                    "Current Password": usr["plaintext_password"] if usr["plaintext_password"] else "— Enforced Hash Override —"
                })
            st.dataframe(pd.DataFrame(raw_user_data), use_container_width=True, hide_index=True)
            
            st.markdown("---")
            
            c_mod1, c_mod2 = st.columns(2)
            with c_mod1:
                section_label("Modify Account Password")
                target_user_edit = st.selectbox("Select user profile", [u["username"] for u in users_list])
                new_pass_val = st.text_input("Type new password string", type="password", key="p_string_val")
                if st.button("Apply Overwrite Password"):
                    if new_pass_val.strip():
                        new_hash = hash_password(new_pass_val)
                        execute_write("UPDATE users SET password_hash = ?, plaintext_password = ? WHERE username = ?", (new_hash, new_pass_val.strip(), target_user_edit))
                        log_activity(f"Administrative override: Changed password credential matrix for account user '{target_user_edit}'")
                        st.success(f"Successfully adjusted credential matrix path for user '{target_user_edit}'.")
                        st.rerun()
                    else:
                        st.error("Field cannot be processed empty.")
                        
            with c_mod2:
                section_label("Terminate User Session Access")
                target_user_del = st.selectbox("Select user to purge", ["— select —"] + [u["username"] for u in users_list if u["username"] != username])
                if st.button("Revoke System Profile Profile", type="primary"):
                    if target_user_del != "— select —":
                        execute_write("DELETE FROM users WHERE username = ?", (target_user_del,))
                        log_activity(f"Administrative override: Completely terminated and deleted system profile access user record: '{target_user_del}'")
                        st.success(f"User account '{target_user_del}' cleanly purged from systemic login directory.")
                        st.rerun()
                        
    st.markdown("---")
    section_label("Global Audit Logs / Trace Records")
    all_logs_view = fetch_all("SELECT timestamp as Timestamp, user as [User Agent], action as [Action Trace Log] FROM system_logs ORDER BY id DESC LIMIT 50")
    if all_logs_view:
        st.dataframe(pd.DataFrame([dict(lx) for lx in all_logs_view]), use_container_width=True, hide_index=True)
    else:
        st.info("System trace index path clear.")
